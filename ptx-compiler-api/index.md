# 1. Introduction — PTX Compiler API 13.1 documentation
...
The APIs accept PTX programs in character string form and create handles to the compiler that can be used to obtain the GPU assembly code. The GPU assembly code string generated by the APIs can be loaded by `cuModuleLoadData` and `cuModuleLoadDataEx`, and linked with other modules by `cuLinkAddData` of the CUDA Driver API or `nvJitLinkAddData` API from [nvjitlink](https://docs.nvidia.com/cuda/nvjitlink/index.html).
...
The main use cases for these PTX Compiler APIs are:
 ...
 - The clients get fine grain control and can specify the compiler [options](index.html#compilation-options) during compilation.
...
This chapter presents the PTX Compiler APIs. Basic usage of the API is explained in Basic Usage.
 ...
 - [PTX-compiler handle](index.html#ptx-compiler-handle)
 - [Error codes](index.html#error-codes)
 - [API Versioning](index.html#api-versioning)
 - [Compilation APIs](index.html#compilation-apis)
...
Register a callback function that the compiler will invoke at different phases of PTX Compilation during a call to nvPTXCompilerCompile() .
...
Note
nvPTXCompilerCompile() API should be invoked for the handle before calling this API. Otherwise, NVPTXCOMPILE_ERROR_COMPILER_INVOCATION_INCOMPLETE is returned.
...
NVPTXCOMPILE_SUCCESS  NVPTXCOMPILE_ERROR_INVALID_INPUT
NVPTXCOMPILE_ERROR_INVALID_COMPILER_HANDLE
...
Option names with two preceding dashes (`--`) are long option names and option names with one preceding dash (`-`) are short option names. Short option names can be used instead of long option names. When a compile option takes an argument, an assignment operator (`=`) is used to separate the compile option argument from the compile option name, e.g., `"--gpu-name=sm_80"`. Alternatively, the compile option name and the argument can be specified in separate strings without an assignment operator, .e.g, `"--gpu-name"``"sm_80"`.
...
>
>
> Under certain situations, `static` device functions can safely inherit a higher register count from the caller entry function. In such cases, ptx compiler may apply the higher count for compiling the static function.
>
>
...
> *Prevents basic block merging, at a slight perfomance cost.*
>
>
...
> *Generate instrumented code with specified compute-sanitizer tool.*
>
>
...
> *Specify level to prefer device code compilation speed.*
>
>
> Default value: `0`.
...
This section of the document uses a simple example, *Vector Addition*, shown in [Figure 1](index.html#ptx-source-string-for-simple-vector-addition) to explain how to use PTX Compiler APIs to compile this PTX program. For brevity and readability, error checks on the API return values are not shown.
...
With this PTX program as a string, we can create the compiler and obtain a handle to it as shown in [Figure 3](index.html#compiler-creation-and-initialization-of-a-program).
...
Compilation can now be done by specifying the compile options as shown in [Figure 4](index.html#compilation-of-ptx-program).
...
The compiled GPU assembly code can now be obtained. To obtain this we first allocate memory for it. And to allocate memory, we need to query the size of the image of the compiled GPU assembly code which is done as shown in [Figure 5](index.html#query-size-of-compiled-assembly-image).
...
The image of the compiled GPU assembly code can now be queried as shown in [Figure 6](index.html#query-the-compiled-assembly-image). This image can then be executed on the GPU by passing this image to the CUDA Driver APIs.
...
When the compiler is not needed anymore, it can be destroyed as shown in [Figure 7](index.html#destroy-the-compiler).
...
# 7. Example: Simple Vector Addition
