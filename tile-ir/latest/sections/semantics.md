...
The Tile IR abstract machine state \(\mathcal{S}\) is a tuple consisting of the following components, each explained below:
 ...
 - A well-formed module \(\mathcal{Mod}\) which stores one of more items, discussed below in [Modules](semantics.html#sub-sec-modules).
...
A program in Tile IR is represented as a module. A module is a single translation unit which contains zero or more items. An item may either be a:
 ...
 - a [global variable definition](semantics.html#sub-sec-tile-global)
 - a [tile kernel](semantics.html#sub-sec-tile-kernel)
 - a [tile function](semantics.html#sub-sec-tile-function)
...
| fp8 (E3M4) | kDLFloat8_e3m4 | 8 | 1 | ml_dtypes.float8_e4m3fn | torch.float8_e4m3fn |
