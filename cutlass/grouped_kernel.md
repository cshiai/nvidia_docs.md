# Grouped Kernel Schedulers[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#grouped-kernel-schedulers "Link to this heading")
...
Unlike a conventional GEMMs in CUTLASS, which launch a number of threadblocks equal to the number of tiles in the GEMM, CUTLASS grouped kernels typically launch a number of threadblocks that is fewer than the total number of tiles across all problems in the group. Each threadblock is then responsible for computing one or more tiles among the problems in the group. The grouped kernel _scheduler_ (referred to as the _problem visitor_ in code) is responsible for assigning each threadblock the sequence of tiles that it will compute within the group.
...
-   [Introduction to Grouped Kernel Schedulers](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#introduction-to-grouped-kernel-schedulers)
-   [Grouped GEMM Scheduler](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#grouped-gemm-scheduler)
-   [Grouped Rank2K Scheduler](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#grouped-rank2k-scheduler)
-   [Scheduler Modes](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#scheduler-modes)
-   [Improving Load Balance by Sorting Problems](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#improving-load-balance-by-sorting-problems)

# Introduction to Grouped Kernel Schedulers[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#introduction-to-grouped-kernel-schedulers "Link to this heading")
...
ProblemVisitor problem\_visitor;
 
while (problem\_visitor.next\_tile()) {
    //
    ...
    problem\_visitor.advance(gridDim.x);
}
...
# Grouped GEMM Scheduler[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#grouped-gemm-scheduler "Link to this heading")
...
![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-gemm-schedule-2x2.png)

A similar mapping for problems that do not have the same number of tiles is shown below:

![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-gemm-schedule-varied.png)

## Computing the schedule for a given block[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#computing-the-schedule-for-a-given-block "Link to this heading")
...
We describe how this search is accelerated in [Scheduler Modes](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#scheduler-modes).

# Grouped Rank2K Scheduler[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#grouped-rank2k-scheduler "Link to this heading")
...
## Inefficiency of grouped GEMM scheduler for grouped Rank2K problems[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#inefficiency-of-grouped-gemm-scheduler-for-grouped-rank2k-problems "Link to this heading")
...
![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-syr2k-schedule-using-grouped-gemm-scheduler.png)
...
## Specializing the scheduler for triangular problems[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#specializing-the-scheduler-for-triangular-problems "Link to this heading")
...
![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-syr2k-schedule-ideal.png)
...
![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-syr2k-schedule-3x3.png)

### Calculating row `i` given threadblock ID `t`[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#calculating-row-i-given-threadblock-id-t "Link to this heading")
...
### Calculating column `j` given threadblock ID `t` and row `i`[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#calculating-column-j-given-threadblock-id-t-and-row-i "Link to this heading")
...
### Accounting for non-square grids[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#accounting-for-non-square-grids "Link to this heading")
...
![ALT](https://docs.nvidia.com/cutlass/latest/_images/grouped-syr2k-schedule-macro.png)
...
### Handling cases with grid dimensions that aren’t multiples of each other[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#handling-cases-with-grid-dimensions-that-aren-t-multiples-of-each-other "Link to this heading")
...
### Handling upper-triangular matrices[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#handling-upper-triangular-matrices "Link to this heading")

The only modification needed for upper-triangular matrices is to swap `i_macro` and `j_macro` in the calculations above.

# Scheduler modes[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#scheduler-modes "Link to this heading")
...
## `GroupScheduleMode::kDeviceOnly` (default)[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#groupschedulemode-kdeviceonly-default "Link to this heading")
...
## Precomputing schedules on the host: `GroupScheduleMode::kHostPrecompute`[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#precomputing-schedules-on-the-host-groupschedulemode-khostprecompute "Link to this heading")

This scheduler attempts to reduce the amount of scheduling performed on the device by precomputing on the host the sequence of problems that will be accessed by each block. As described above, all that is needed to map tile\_idx to the specific tile within a problem to compute is the problem ID and the problem’s starting tile (among all of the tiles in the group). Thus, this scheduler precomputes the problem index and problem starting tile for each tile computed by each block.
...
## Which scheduler mode should I use?[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#which-scheduler-mode-should-i-use "Link to this heading")

Consider the following questions when deciding which scheduling mode to use:

### How are the parameters used as input to the grouped kernel (e.g., ptrA, lda) set in my application?[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#how-are-the-parameters-used-as-input-to-the-grouped-kernel-e-g-ptra-lda-set-in-my-application "Link to this heading")
...
### Can host-side work be overlapped with other device kernels in my application?[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#can-host-side-work-be-overlapped-with-other-device-kernels-in-my-application "Link to this heading")
...
### How compute-intensive are the problems in my group?[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#how-compute-intensive-are-the-problems-in-my-group "Link to this heading")
...
# Improving Load Balance by Sorting Problems[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#improving-load-balance-by-sorting-problems "Link to this heading")
...
## Potential problems with imbalanced K dimension[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#potential-problems-with-imbalanced-k-dimension "Link to this heading")
...
## Potential for sorting problems to reduce imbalance[#](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html#potential-for-sorting-problems-to-reduce-imbalance "Link to this heading")
