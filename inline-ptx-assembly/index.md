# 1. Using Inline PTX Assembly in CUDA — Inline PTX Assembly in CUDA 13.1 documentation
...
The *constant-expression* is evaluated during compilation and shall generate the address of a variable `V`, where:


 - `V` has *static storage duration*.
 - `V` has type ‘array of const char’.
 - `V` is *constant-initialized*.
 - If `V` is a static class member, then `V`’s *initializing declaration* is the declaration within the class.


During translation, the compiler will replace a reference to the operand within the *Assembler Template* with the contents of `V`’s initializer, except for the last trailing zero. No constraint modifiers are allowed for this constraint. This constraint can only be used in device code.
