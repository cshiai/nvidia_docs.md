# 1. Introduction — NVVM IR Specification 13.1 documentation
...
This document describes version 2.0 of the NVVM IR and version 3.1 of the NVVM debug metadata (see [Source Level Debugging Support](index.html#source-level-debugging-support)). The 2.0 version of NVVM IR is incompatible with the previous version 1.11. Linking of NVVM IR Version 1.11 with 2.0 will result in compiler error.


NVVM IR can be in one of two dialects. The LLVM 7 dialect is based on LLVM 7.0.1. The modern dialect is based on a more recent public release version of LLVM (LLVM 20.1.0). The modern dialect only supports Blackwell and later architectures (compute capability `compute_100` or greater). For the complete semantics of the IR, readers of this document should refer to either the official LLVM Language Reference Manual [version 7](https://releases.llvm.org/7.0.1/docs/LangRef.html) or [version 20](https://releases.llvm.org/20.1.0/docs/LangRef.html). This document is annotated with notes when differences between the two NVVM IR dialects are important.
...
See [NVVM ABI for PTX](index.html#nvvm-abi-for-ptx) for details on how linkage types are translated to PTX.
...
If no address space is explicitly specified, the global variable is assumed to reside in the `global` address space with a generic address value. See [Address Space](index.html#address-space) for details.
...
Accepted and ignored, except for the following:
 ...
 - `!nvvm.annotations`: see [Global Property Annotation](index.html#global-property-annotation-chapter-11)
 - `!nvvmir.version`
 - `!llvm.dbg.cu`
 - `!llvm.module.flags`
...
See [Calling Conventions](index.html#calling-conventions) for the use of the attributes.
...
The following metadata are understood by the NVVM compiler:
 ...
 - `callalign` (see [Rules and Restrictions](index.html#rules-and-restrictions) for Calling Conventions)
...
See [Conversion](index.html#conversion) for a special use case of `bitcast`.
...
Supported:
 ...
 - `select`
 - `va_arg`
 - `call` (See [Calling Conventions](index.html#calling-conventions) for other rules and restrictions.)
...
| --- | --- | --- |
| code | 0 | functions, code

CUDA C/C++ function
OpenCL C function |
| generic | 0 | Can only be used to qualify the pointee of a pointer

Pointers in CUDA C/C++ |
| global | 1 | CUDA C/C++ __device__
OpenCL C global |
| shared | 3 | CUDA C/C++ __shared__
OpenCL C local |
| constant | 4 | CUDA C/C++ __constant__
OpenCL C constant |
| local | 5 | CUDA C/C++ local
OpenCL C private |
...
See [Accessing Texture Memory or Surface Memory](index.html#accessing-texture-memory-or-surface-memory) for details.
...
NVVM IR debugging support is based on that in LLVM 7.0.1 for pre-Blackwell targets and LLVM 20.1.0 for Blackwell and later targets. For the complete semantics of the IR, readers of this chapter should refer to the official LLVM IR [Specialized Metadata Nodes](https://releases.llvm.org/7.0.1/docs/LangRef.html#specialized-metadata-nodes) and the [Source Level Debugging](https://releases.llvm.org/7.0.1/docs/SourceLevelDebugging.html) documents. Blackwell and later targets should refer to [this](https://releases.llvm.org/20.1.0/docs/LangRef.html#specialized-metadata-nodes) and [this](https://releases.llvm.org/20.1.0/docs/SourceLevelDebugging.html) document respectively.
...
Version 1.1
 ...
 - Added support for UVM managed variables in global property annotation. See [Supported Properties](index.html#supported-properties).
